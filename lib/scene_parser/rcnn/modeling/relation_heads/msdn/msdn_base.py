...
class MSDN_BASE(nn.Module):
    def __init__(self, fea_size, dropout=False, gate_width=128, use_region=False,
    ...
    def forward(self, feature_obj, feature_phrase, feature_region, mps_object, mps_phrase,
                mps_region):
    ...
    def prepare_message(self, target_features, source_features, select_mat, gate_module):
        ...
        if select_mat.data.sum() == 0:
            temp = Variable(torch.zeros(target_features.size()[1:]), requires_grad=True).type_as(
                target_features)
