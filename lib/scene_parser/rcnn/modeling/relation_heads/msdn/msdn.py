...
class MSDN(MSDN_BASE):
    def __init__(self, cfg, in_channels, dim=1024, dropout=False, gate_width=128,
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        for t in range(self.update_step):
            ...
            object_sub = self.prepare_message(x_obj[t], x_pred[t], subj_pred_map,
                                              self.gate_pred2sub)
        ...
        return (x_obj[-1],
                x_pred[-1]), obj_class_logits, pred_class_logits, obj_class_labels, rel_inds
