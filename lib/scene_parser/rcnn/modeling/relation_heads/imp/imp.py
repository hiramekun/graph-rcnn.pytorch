...
class IMP(nn.Module):
    ...
    def _get_map_idxs(self, proposals, proposal_pairs):
        ...
        subj_pred_map = rel_inds.new(sum([len(proposal) for proposal in proposals]),
                                     rel_inds.shape[0]).fill_(0).float().detach()
        obj_pred_map = rel_inds.new(sum([len(proposal) for proposal in proposals]),
                                    rel_inds.shape[0]).fill_(0).float().detach()
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        for t in range(self.update_step):
            ...
            message_pred_to_subj = self.subj_node_gate(torch.cat([sub_vert, hx_edge[t]], 1)) * \
                                   hx_edge[t]  # nrel x d
            message_pred_to_obj = self.obj_node_gate(torch.cat([obj_vert, hx_edge[t]], 1)) * \
                                  hx_edge[t]  # nrel x d
            node_message = (torch.mm(subj_pred_map, message_pred_to_subj) / (
                        subj_pred_map.sum(1, keepdim=True) + 1e-5) \
                            + torch.mm(obj_pred_map, message_pred_to_obj) / (
                                        obj_pred_map.sum(1, keepdim=True) + 1e-5)) / 2.
            ...
            message_subj_to_pred = self.subj_edge_gate(
                torch.cat([sub_vert, hx_edge[t]], 1)) * sub_vert  # nrel x d
            message_obj_to_pred = self.obj_edge_gate(
                torch.cat([obj_vert, hx_edge[t]], 1)) * obj_vert  # nrel x d
        ...
        return (hx_obj[-1],
                hx_edge[-1]), obj_class_logits, pred_class_logits, obj_class_labels, rel_inds
